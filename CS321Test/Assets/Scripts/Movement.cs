...
public class Movement : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag == rb2d.tag)
        {
            ...
            //AudioManager.instance.PlaySingle(onGoal, 0.1f);
            if (rb2d.tag == "red")
            {
                background.GetComponent<BoardManager>().setGoalRed(true);
               // Debug.Log("set red goal true");
            }
            if (rb2d.tag == "green")
            {
                background.GetComponent<BoardManager>().setGoalGreen(true);
               // Debug.Log("set green goal true");
            }
        }
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.tag == rb2d.tag)
        {
            ...
            //  Debug.Log("set " + rb2d.tag + " goal false");
            background.GetComponent<BoardManager>().setGoalRed(false);
            background.GetComponent<BoardManager>().setGoalGreen(false);
        }
    }

    protected IEnumerator SmoothMovement(Vector3 end) //CREDIT THIS
    {
